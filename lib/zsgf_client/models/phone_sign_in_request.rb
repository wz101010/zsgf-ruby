  ...
  class PhoneSignInRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ZSGFClient::PhoneSignInRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ZSGFClient::PhoneSignInRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'verify_code')
        self.verify_code = attributes[:'verify_code']
      else
    ...
    def list_invalid_properties
      ...
      if @verify_code.nil?
        invalid_properties.push('invalid value for "verify_code", verify_code cannot be nil.')
      end

      if @verify_code.to_s.length < 1
    ...
    # Custom attribute writer method with validation
    # @param [Object] verify_code Value to be assigned
    def verify_code=(verify_code)
      if verify_code.nil?
        fail ArgumentError, 'verify_code cannot be nil'
      end
    ...
    def hash
      [phone, verify_code, two_factor_code].hash
    end
